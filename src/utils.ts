...
export const saveDismissalState = ( storageKey: string = DEFAULT_DISMISSAL_KEY ) =>
{
  if ( typeof window === "undefined" ) return;
  localStorage.setItem( storageKey, "true" );
};

export const clearDismissalState = ( storageKey: string = DEFAULT_DISMISSAL_KEY ) =>
{
  if ( typeof window === "undefined" ) return;
  localStorage.removeItem( storageKey );
};

export const isWalkthroughDismissed = ( storageKey: string = DEFAULT_DISMISSAL_KEY ) =>
{
  if ( typeof window === "undefined" ) return false;
  return localStorage.getItem( storageKey ) === "true";
};

export const findElementByDataHighlight = ( highlightValue: string ): HTMLElement | null =>
{
...
export const calculateTooltipPosition = (
  element: HTMLElement,
  preferredPosition: WalkthroughTooltipPosition,
  tooltipWidth = 320,
  tooltipHeight = 150
): TooltipCoordinates =>
{
  ...
  let top = 0;
  let left = 0;
  let position: WalkthroughTooltipPosition = preferredPosition;
  ...
  if ( top < scrollTop ) top = scrollTop + 16;
  if ( top + tooltipHeight > scrollTop + viewportHeight )
  {
